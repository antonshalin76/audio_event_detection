...
- Upd. Сделал вариант детекции не по сохраненным файлам, а <a href="https://github.com/antonshalin76/audio_event_detection/blob/main/Audio_stream_event_detection.ipynb">потоковую детекцию событий с микрофона</a>. Работает на локальной машине, не в colab облаке, как прошлый вариант с файлами. Запуск детекции сразу по выполнению ячейки в Jupyter Notebook, прекращение по нажатию кнопки оостановки. Результат оставил в том же виде: накапливающийся перечень событий с метками реального времени и сохранение накопленного перечня в csv файл после остановки процесса детекции.
